...
function mergeWorkflowSettings(current, override) {
  if (override.routing !== undefined) {
    ...
    for (const key of Object.keys(override.routing)) if (!['mode', 'allowFableHeadless', 'pins', 'phases'].includes(key)) throw new Error(`지원하지 않는 routing 설정: ${key}`);
    ...
    const phases = override.routing.phases === undefined ? {} : override.routing.phases;
    object(phases, 'routing.phases');
    const phaseTasks = { plan_review: 'review', implement: 'implement', code_review: 'review' };
    for (const [phase, profile] of Object.entries(phases)) {
      if (!Object.hasOwn(phaseTasks, phase) || !allowed[phaseTasks[phase]].includes(profile)) throw new Error(`지원하지 않는 routing phase: ${phase}/${profile}`);
    }
    current.routing = { ...current.routing, ...override.routing, pins: { ...current.routing.pins, ...pins }, phases: { ...current.routing.phases, ...phases } };
  }
...
// The positional snapshot is supplied only by package launch code, never from tool options.
export function buildPolicy(role, { cwd = process.cwd(), configCwd = cwd, effort, skills: selectedSkills = [] } = {}, settingsSnapshot) {
  const targetCwd = directory(cwd);
  const originCwd = directory(configCwd);
  const { roles, defaults, config: configPath } = settingsSnapshot ?? roleState(originCwd);
...
export function buildLaunch(options, settingsSnapshot) {
  ...
  if (role === 'cold-read') {
    ...
    const roles = settingsSnapshot?.roles ?? readRoles(configCwd);
    ...
    const { config, policy, sources: policySources } = buildPolicy(role, { cwd, configCwd, effort: options.effort, skills: options.skills }, settingsSnapshot);
