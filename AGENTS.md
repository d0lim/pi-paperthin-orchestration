...
- 작업 요청은 `/lead <원하는 결과>`로 전달한다. `/lead`만 입력하면 여러 줄 입력창이 열리고, `/lead --help`는 사용법을 보여준다. 일반 대화로 전달한 목표에도 같은 Lead 운영 지침을 적용한다. 브리프와 worktree 준비는 Lead가 맡는다.
...
- 승인된 작업 범위는 계속 수행한다. 이미 허용된 읽기·수정·테스트마다 재확인을 요청하지 않는다. 구현 요청에는 작업용 worktree 준비, 담당 변경의 로컬 후보 커밋, 리뷰 후 로컬 통합을 포함한다. 사용자가 정한 제외 범위와 커밋 제한을 우선한다. push·원격 PR 병합·공개 배포·파괴적 작업은 해당 작업에 대한 사용자 권한이 필요하다.
